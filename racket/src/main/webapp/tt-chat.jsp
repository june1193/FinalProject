...
var wsocket;
	...
	$(document).ready(function(){
		...
		//소켓 연결
		function connect(){
			wsocket = new WebSocket("ws://3.107.70.76:8080/racket/chat-wsTT");
		...
		var disConnect = false;
		 function disconnect() {
		    	...
		    	$.ajax({
		    		type : "get",
		    		url : "/racket/pingpong/deleteNickname/"+nickname,
		    		success:function(data){
			    	   ...
			    	   event.preventDefault();
				        ...
				        $('#entrance').click(function() { 
				        	...
				        	if(nickname == ''){
								...
								$.ajax({
						    		type : "get",
						    		url : "/racket/pingpong/checkNickname/"+nickname,
						    		success:function(data){
						    			if(data > 0){
...
.send{
 ...
 .window:before{
 	background: linear-gradient(transparent 20%,#ffffffb3 40%,transparent 41%),linear-gradient(90deg,#ffffff66,#0000001a,#ffffff33),#b4f583;
 }
 .title-bar{
 	background: linear-gradient(90deg, #ffffff66, #0000001a, #ffffff33), #b4f583;
 }
 ...
 background : linear-gradient(180deg, #fce5f9, #f8ade1 30% 50%, #f285c1 50%, #b4f583);
 }
 .window-body{
 background: #f1ffed;
 }
...
<body>
	<div id="wrap">
		<div class="window active" style="max-width: 400px">
			<div class="title-bar">
				<div class="title-bar-text">탁구를 사랑하는 사람들의 채팅방</div>
